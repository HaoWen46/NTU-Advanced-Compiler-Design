...
class CFG:
    ...
    def construct_cfg(self) -> BasicBlock:
        ...
        for instr in self.function.instrs:
            if hasattr(instr, 'label'):
                if prev_instr == None or prev_instr.op not in ['br', 'jmp', 'ret']:
                    current_block.instructions.append(EffectOperation({'op': 'jmp', 'args': [], 'funcs': [], 'labels': [instr.label]}))
        ...
        if self.function.type == None:
            for block in [entry_block] + list(self.blocks.values()):
                if not block.successors and (not block.instructions or block.instructions[-1].op != 'ret'):
                    block.instructions.append(EffectOperation({'op': 'ret', 'args': [], 'funcs': [], 'labels': []}))
